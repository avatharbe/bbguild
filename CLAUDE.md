...
- **Version:** 2.1.0 (stable; version lives in `ext::BBGUILD_VERSION`, not `phpbb_config`)
...
Full plan: **`contrib/roadmap-2.x.md`** (parity matrix + release plan); also published on the docs site at https://avatharbe.github.io/bbguild/. The old standalone BBCode forum status post (`contrib/roadmap-2.x-forum-post.txt`) was removed — forum announcements are now per-release, not a standing status post.

- **North star:** feature parity vs. phpBB native / legacy bbDKP MOD / the extension family / guildsofwow.com. Releases are a **coordinated train** across core + all 9 game plugins — shipped together on the same milestone dates and **pairing-locked** (every plugin hard-requires the current core version via `ext.php::is_enableable()`). **2.1.0 tagged 2026-09-22** (core `2.1.0`; bbguildwow already one patch ahead at `2.1.1`, see its own CLAUDE.md) — milestone fully closed, 0 open issues, migration squash #383 shipped. **2.0.0 shipped stable 2026-09-12** (core `2.0.0`; bbguildwow's pairing bumped to require `>=2.0.0`) — the rc5→stable step was a version-number/pairing-lock bump only, no schema or code change (#244, the sole 2.0.0 blocker, closed 2026-09-12).
- **Milestone scheme (GitHub, milestone-driven versioning — no version labels):**
  ...
  - `2.1.0` (due 2026-10-15, **shipped, tagged `v2.1.0` 2026-09-22**) — **guild page overhaul**: page-level portal tabs (#360, **shipped**, closed 2026-09-13), character-sync scheduler contract (#361, **shipped**) + WoW handler (#362, **shipped**), gear tooltips via bbTips + bonus IDs (#363, **shipped** — implemented under bbguildwow's `2.1.0-b1` line, closed 2026-09-12), character page polish (#364, **shipped**, closed 2026-09-18), per-character achievements (#365, **shipped** as a tracking issue, closed 2026-09-18), guild statistics portal module (#366, **shipped**), character-based forum avatars (#369, restores pbwowext#10 pt1, **shipped**), about page popup + footer credit (#373, **shipped**, closed 2026-09-13), bbGuild event catalogue phase 1 (#370, in-process events only — **shipped**, merged via PR #378, `@since 2.1.0` since it ships this train rather than waiting for 2.3.0; see below), UCP character-add 503 fix (#377, **shipped**, closed 2026-09-13); player-detail sub-tab bar, core half (#375, **shipped**, merged via PR #376); unit test coverage follow-up (#372, split from #244, **shipped**, closed 2026-09-18); plugin spec data (#367, **closed as not planned** 2026-09-18 — scope undefined, superseded in practice by the per-plugin spec-seeding sub-issues, all 9 of which shipped independently by 2026-09-16) + **GW2 API v2 sync** (bbguildgw2#9); migration squash #383 (**shipped**, `2ec57f25`, see below).
  - `2.2.0` (due 2026-11-30) — Events/RSVP calendar (new ext — Raidplanner MOD prior art has guild-specific raid signup/roster + bbDKP integration a generic third-party calendar extension doesn't cover, so still being built), roster↔profile fields (#231), profile-field character info (#368, pbwowext#10 pt2), professions (#230), player stats (#289), guild-statistics chart visualization + build list (#279, reopened 2026-09-14 — moved out of 2.1.0's #366 scope, see below), guild activity feed portal module (#388), roster image fallback (#389 — **shipped** 2026-09-24 via PR #390, the root-cause fix behind the plugin icon cluster; see `contrib/roadmap-2.x.md`'s icon-cluster subsection), `<game>_unknown.png` fallback icons across all 9 plugins (#391 — needs authoring, no upstream source).
  - `2.3.0` (due 2027-01-15) — **bbGuild API surface, read-API half only (#370)** — the event-catalogue half already shipped in 2.1.0 (see above); this milestone now covers just the out-of-process read API (auth/versioning/rate-limiting, separate spec), Discord (new ext, open question, not decided), Gameworld port (existing repo, old phpBB 3.0 MOD → 3.3 extension, `Gameworld#16`, milestoned 2.3.0), Battle.net API modernisation, spec build analysis (#286).
...
- **Docs site:** `contrib/` (architecture, database, roadmap, changelog, install guide, events) now also publishes as a browsable MkDocs site at https://avatharbe.github.io/bbguild/, built via GitHub Actions on every push to `contrib/`/`mkdocs.yml`; README points there. Community health files (CODE_OF_CONDUCT, SECURITY, CONTRIBUTING, issue/PR templates) added, deferring to the existing house policy (forum topic 1854) rather than a separate process.
...
- **#377 UCP character-add 503** - `model/player/player.php`'s constructor never initialized `$deactivate_reason`; the UCP add flow (`Makeplayer()` → `Updateplayer()`) never calls `Getplayer()` in between (the only place that otherwise sets it), so `Updateplayer()`'s `UPDATE` wrote `null` into a `NOT NULL` column on every character add. `Makeplayer()` now defaults it to `''` when null, matching the existing convention already used there for `player_status`/`player_title`. Reproduced the exact SQL error first, then confirmed the fix, via a direct exercise of the real `player` model with the same field-setting sequence the UCP add flow uses.
- **bbguildwow#53 guild-deletion portal-tab leak** - `delete_guild()` (`model/player/guilds.php`) only ever deleted from `bb_ranks`/`bb_guild`, never `bb_portal_tabs`/`bb_portal_modules`. Since guild ids are reassigned via `MAX(id)+1` (not a true auto-increment), deleting a guild and later re-adding one that lands back on the freed id — guaranteed for id=1 once the table is emptied — collided with the orphaned `portal_tabs` row on its `(guild_id, tab_slug)` unique key. Added `database_handler::delete_guild_layout($guild_id)`, called alongside `guilds::delete_guild()` from `admin_guild::DeleteGuild()`.
- **ACP dashboard guild count** - `admin_main.php`'s "number of guilds" stat did a bare `COUNT(*)` on `bb_guild`, which includes the reserved `guild_id=0` "Guildless" placeholder row — an install with zero real guilds showed "1". Now excludes `id=0`.
...
### Phase 3: 2.1.0 — COMPLETE (2.1.0, tagged 2026-09-22)
...
- #364 — Character page polish (layout + async stats) — **shipped**, closed 2026-09-18. Async `/bbguildwow/character-stats/{player_id}` endpoint replaces 4 synchronous Battle.net calls; layout redesign (hero render in header, two-column equipment, stat cards) shipped via #375's Character tab restyle.
- #365 — Per-character achievements view — **shipped** as a tracking issue, closed 2026-09-18; concrete implementation continues in bbguildwow#44.
- #366 — Guild statistics portal module — **shipped** (5b04fe69, closed 2026-09-12; `portal.module.statistics` tagged as a built-in module in `config/portal_services.yml`). Delivered the numbers/percentage lists (class/race/level distribution) but not the chart/graph visualization or optional build-list #279 also asked for — #279 reopened 2026-09-14 and moved to 2.2.0 for that remaining scope; bbDKPMOD's `dkp/view/stats_classes.html` (jQuery Flot pie charts for % members / % loot by class) is real prior art for the chart half, though its loot/DKP data stays out of scope per bbDKP v2
- #367 — Complete plugin spec data for 8 non-WoW games — **closed as not planned**, 2026-09-18 (audited all 8 plugins against bbguildwow as reference; found the games already have real, researched data — several intentionally-empty `spec_catalog()` returns are cited/documented decisions, not gaps; the one confirmed gap is thinner translation coverage vs. WoW's 4 languages. Superseded in practice by the per-plugin spec-seeding sub-issues below, all of which shipped independently.)
- #369 — Character-based forum avatars, restores pbwowext#10 pt1 — **shipped**
- #370 — bbGuild event catalogue, phase 1 (in-process events only) — **shipped**, merged [PR #378](https://github.com/avatharbe/bbguild/pull/378) 2026-09-14; 12 new `trigger_event()` call sites (character CRUD/claim/unclaim, sync completion, roster/portal render, recruitment, MOTD) + retroactive docs for the 10 pre-existing events + new `contrib/Events.md` catalogue mirroring `avathar/recenttopics`'s convention. Tagged `@since 2.1.0` (ships this train, not 2.3.0 as originally scoped). #370 itself stays open under milestone 2.3.0 for the remaining read-API phase 2 work — not a separate not-yet-filed issue, despite this doc previously implying that.
- #372 — Unit test coverage: migrations + remaining guild/player CRUD paths, split from #244 — **shipped**, closed 2026-09-18. Migration idempotency test, guild CRUD success-path tests, and player CRUD beyond claim/unclaim all landed in `a3864af8` (102 local tests, CI matrix green as of `b1842bd2`). ACP/UCP controller-level coverage stays explicitly out of scope, tracked separately.
- #373 — About page popup + footer copyright credit — **shipped**, closed 2026-09-13. `/about` route (headerless popup via `simple_header`/`simple_footer`), footer link, credits pulled from the existing (previously unused) bbDKP/EQDkp-era language strings, current maintainer shown by handle only.
- #374 — New guilds got an empty portal (no roster/MOTD) — **shipped**. `seed_guild_layout()` copies its default layout from `guild_id=0`, but no migration had ever populated that template (only the removable `guild_id=1` demo guild from `v200b3` had one). Fixed via migration `v210b3/fix_default_portal_template`, which seeds the `guild_id=0` template directly and backfills any existing guild that still has zero tabs. Found via Bête Noire (`guild_id=2`) rendering a completely empty portal despite active roster sync.
- #375 — Player-detail sub-tab bar, core half — **shipped**, merged [PR #376](https://github.com/avatharbe/bbguild/pull/376) 2026-09-13. New `player_detail_tab_interface` + `bbguild.player_detail_tab` tagged-service collection (`player_detail_tab_registry`) let game plugins register additional player-detail tabs; `view_controller`/routing extended with an optional tab slug, Character stays core's built-in first tab. bbguildwow's restyled Character tab + new Talents/Raid Progression/PVP tabs are the first consumer (its own changelog/roadmap).
- #377 — UCP character-add 503 (`player.php` never initialized `$deactivate_reason`, `Makeplayer()`→`Updateplayer()` writes null into a `NOT NULL` column) — **shipped**, closed 2026-09-13, `3e2c1d3d`. Found during #370's manual verification pass, unrelated pre-existing bug.
- #279 — Guild statistics: chart/graph visualization + optional build list — reopened 2026-09-14, moved to milestone 2.2.0. Was closed as superseded by #366, but #366 only ever delivered the numbers/percentage list half of #279's ask; the charting and build-list parts were never built. See #366 above for prior-art notes (bbDKPMOD's Flot pie charts).
- #380 — Unify guild portal-block panes with the player-detail pane style — **shipped**, `22b9f0f9`, closed 2026-09-21. `_block_config.html`'s `C_BLOCK_*`/`LR_BLOCK_*` wrappers rewritten to emit `.player-detail-pane`/`.player-detail-pane-title` markup instead of the old Board3 `forabg`/`topiclist` wrapper, so every portal module reads as one visual system across the guild front page and character/talents/achievements/pvp pages.
- #381 — Migrate legacy free-text `player_spec` text → `player_spec_id` — split out of #331 (closed at 2.0.0-rc4, Phase 5 left as an unchecked todo there) into its own issue, **pulled into 2.1.0**, then **closed without a migration**: existing installs aren't a support concern (clean install is the path forward), so there's no real backlog to backfill in practice.
- #382 — Newly-registered/banned accounts selectable in the ACP character-link "Username" dropdown — **shipped**, closed 2026-09-21. Dropdown only excluded BOTS/GUESTS (hardcoded ids); added `NEWLY_REGISTERED` and `banned`, both resolved by group name at request time rather than hardcoded ids (only GUESTS/BOTS are guaranteed-stable ids across installs).
- #383 — Squash 2.1.0 migrations (`v210b1`/`v210b2`/`v210b3`, plus anything else that lands) into a single one before release, matching the 2.0.0 train's squash at `v200b3` — **shipped**, `2ec57f25`, closed 2026-09-22. Existing installs aren't a concern (clean install is the supported path), so no need to preserve fine-grained migration history for this train.
- ACP roster listing: split the combined Race/Class column into separate **Race** / **Class** / **Spec** columns (each with its icon where available), reusing the same spec-resolution + legacy-text-fallback pattern as the front-end roster's `resolve_spec()`. Also added the previously-missing race-name join to `guilds::list_players()` (only the race image was ever shown before). — **shipped**, `788db9d4`.
- #331 Phase 3c — Recruitment spec filter (not started, 2.2.0)
- All 9 plugin Phase 4 spec-seeding sub-issues (eq, eq2, ffxi, ffxiv, gw2, lineage2, lotro, swtor, bbguildwow#26 for WoW) — **shipped**, all closed by 2026-09-16 (gw2's shipped earlier, 2026-07-24). Supersedes this doc's previous "8 of 9 open" note.
