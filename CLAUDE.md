...
- #367 — Complete plugin spec data for 8 non-WoW games — **parked** 2026-09-13 (audited all 8 plugins against bbguildwow as reference; found the games already have real, researched data — several intentionally-empty `spec_catalog()` returns are cited/documented decisions, not gaps; the one confirmed gap is thinner translation coverage vs. WoW's 4 languages, but scope beyond that wasn't well-defined — labeled `inactive`, needs a concrete re-scope before resuming)
- #369 — Character-based forum avatars, restores pbwowext#10 pt1 — **shipped**
- #370 — bbGuild event catalogue, phase 1 (in-process events only) — **shipped**, merged [PR #378](https://github.com/avatharbe/bbguild/pull/378) 2026-09-14; 12 new `trigger_event()` call sites (character CRUD/claim/unclaim, sync completion, roster/portal render, recruitment, MOTD) + retroactive docs for the 10 pre-existing events + new `contrib/Events.md` catalogue mirroring `avathar/recenttopics`'s convention. Tagged `@since 2.1.0` (ships this train, not 2.3.0 as originally scoped). Read-API phase 2 remains a separate future spec under the 2.3.0 milestone.
...
- #380 — Unify guild portal-block panes with the player-detail pane style — **shipped**, `22b9f0f9`. `_block_config.html`'s `C_BLOCK_*`/`LR_BLOCK_*` wrappers rewritten to emit `.player-detail-pane`/`.player-detail-pane-title` markup instead of the old Board3 `forabg`/`topiclist` wrapper, so every portal module reads as one visual system across the guild front page and character/talents/achievements/pvp pages.
- #381 — Migrate legacy free-text `player_spec` text → `player_spec_id` — split out of #331 (closed at 2.0.0-rc4, Phase 5 left as an unchecked todo there) into its own issue and **pulled into 2.1.0** (was informally tracked as 2.2.0). Confirmed on production data: every character in the Bête Noire guild currently has `player_spec_id = 0` with only the legacy `player_spec` text set — this is why the ACP character-edit spec dropdown shows nothing selected for existing characters, and why the ACP roster's new Spec column (below) currently falls back to plain text with no icon for all of them.
- #331 Phase 3c — Recruitment spec filter (not started, 2.2.0)
- ACP roster listing: split the combined Race/Class column into separate **Race** / **Class** / **Spec** columns (each with its icon where available), reusing the same spec-resolution + legacy-text-fallback pattern as the front-end roster's `resolve_spec()`. Also added the previously-missing race-name join to `guilds::list_players()` (only the race image was ever shown before). `acp/player_module.php`, `model/player/guilds.php`, `adm/style/acp_listplayers.html` — not yet committed.
