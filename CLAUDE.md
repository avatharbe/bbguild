...
| Status | 2.0.0 stable shipped 2026-09-12; 2.1.0 (guild page overhaul) feature-complete, migration squash (#383) the only thing pending before release |
...
### Phase 3: 2.1.0 — feature-complete, #383 (migration squash) the only thing left before release
...
- #383 — Squash 2.1.0 migrations (`v210b1`/`v210b2`/`v210b3`, plus anything else that lands) into a single one before release, matching the 2.0.0 train's squash at `v200b3` — **open, last thing blocking release**. Existing installs aren't a concern (clean install is the supported path), so no need to preserve fine-grained migration history for this train.
