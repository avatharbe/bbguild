...
- #370 — bbGuild event catalogue, phase 1 (in-process events only) — **shipped to PR**, [#378](https://github.com/avatharbe/bbguild/pull/378) open against main; 12 new `trigger_event()` call sites (character CRUD/claim/unclaim, sync completion, roster/portal render, recruitment, MOTD) + retroactive docs for the 10 pre-existing events + new `contrib/Events.md` catalogue mirroring `avathar/recenttopics`'s convention. Tagged `@since 2.1.0` (ships this train, not 2.3.0 as originally scoped). Read-API phase 2 remains a separate future spec under the 2.3.0 milestone.
